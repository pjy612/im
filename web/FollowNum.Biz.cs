...
{
    /// <summary>主播关注数</summary>
    [ModelCheckMode(ModelCheckModes.CheckTableWhenFirstUse)]
    public class FollowNum : FollowNum<FollowNum> { }
    
    /// <summary>主播关注数</summary>
    public partial class FollowNum<TEntity> : Entity<TEntity> where TEntity : FollowNum<TEntity>, new()
    {
        #region 对象操作
            ﻿
        static FollowNum()
        ...
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static TEntity FindByUid(Int64 uid)
        {
            if (Meta.Count >= 1000)
                return Find(__.Uid, uid);
            else // 实体缓存
                return Meta.Cache.Find(e => e.Uid == uid);
            // 单对象缓存
