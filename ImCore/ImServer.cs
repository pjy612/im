...
class ImServer : ImClient
{
    ...
    public ImServer(ImServerOptions options) : base(options)
    {
        ...
        new TimerX(state => { Console.WriteLine($"当前客户数：{_clients.Count}"); }, null, 1000, 5000) {Async = true};
        new TimerX(state => { _redis.Del("preBlack", "blacklist"); }, null, 100, 10 * 60_000) {Async    = true};
    ...
    private readonly Lazy<WorkQueue<Action>> lazyQueue = new Lazy<WorkQueue<Action>>(() =>
    {
        WorkQueue<Action> q = new WorkQueue<Action>(3);
        q.Process += (body) =>
        {
            body();
        };
